    ...
    intersectLine(l: Segment): SegmentIntersection {
        ...
        if (denom !== 0) {
            ...
            if (ua >= 0.0 && ua <= 1.0 && ub >= 0.0 && ub <= 1.0) {
                isec = new SegmentIntersection(SegmentIntersectionType.INTERSECTING, this.a.interpolateTo(this.b, ua));
            } else {
                isec = new SegmentIntersection(SegmentIntersectionType.NON_INTERSECTING, this.a.interpolateTo(this.b, ua));
            }
        } else {
            if (na === 0 && nb === 0) {
...
// https://web.archive.org/web/20230201132228/https://brunoimbrizi.com/unbox/2015/03/offset-curve/
function offsetQuadraticCurve(curve: QuadraticCurveProperties, distance:number = 30, useSplitCurve: boolean = true): QuadraticCurveProperties[] {
    ...
    if (!split) { // Curve angle > 90 degrees
