...
import { type RenderDrawingConfig, type RenderProgramConfig } from './render';
...
export const SvgRenderProgramConfig: RenderProgramConfig = {
	...
	cut: {
		begin: () => '<g class="cut">',
		rapidTo: (cut: Cut) =>
			`<line class="rapid" x1="${cut.rapidTo?.startPoint.x}" y1="${cut.rapidTo?.startPoint.y}" x2="${cut.rapidTo?.endPoint.x}" y2="${cut.rapidTo?.endPoint.y}" />`,
		end: () => '</g>'
	},
}
...
export const SvgRenderDrawingConfig: RenderDrawingConfig = {
	...
	chain: {
		begin: () => '<g class="chain">',
		startPoint: (chain: Chain) =>
			`<circle class="chain start point" cx="${chain.startPoint.x}" cy="${chain.startPoint.y}"/>`,
		endPoint: (chain: Chain) =>
			`<circle class="chain end point" cx="${chain.endPoint.x}" cy="${chain.endPoint.y}"/>`,
		end: () => '</g>'
	},
	shape: {
