...
function parsePathElement(element): Shape[] {
	...
	const commands = svgPathParser.makeAbsolute(svgPathParser.parseSVG(pathD));
	...
	for (const element of layerChildren) {
		if (element.tagName == 'g') {
			...
			if (svgTransformMatrix) {
				...
				for (const shape of elementShapes) shape.transform(matrix);
			}
			shapes.push(...elementShapes);
		}
	}
...
export class SvgFile {
	from(content: string): InputDrawing {
