...
import { type QuadraticCurveProperties } from './quadratic-curve';
...
export function transformQuadraticCurve(
...
export function offsetQuadraticCurve(
	curve: QuadraticCurveProperties,
	distance: number,
	samples: number = 100
) {
	...
	for (var i = 0; i <= samples; i++) {
		...
		var Bx =
			Math.pow(1 - t, 2) * curve.startPoint.x +
			2 * (1 - t) * t * curve.control1.x +
			Math.pow(t, 2) * curve.endPoint.x;
		var By =
			Math.pow(1 - t, 2) * curve.startPoint.y +
			2 * (1 - t) * t * curve.control1.y +
			Math.pow(t, 2) * curve.endPoint.y;

		// Compute derivative B'(t)
		var Bdx =
			2 * (1 - t) * (curve.control1.x - curve.startPoint.x) +
			2 * t * (curve.endPoint.x - curve.control1.x);
		var Bdy =
			2 * (1 - t) * (curve.control1.y - curve.startPoint.y) +
			2 * t * (curve.endPoint.y - curve.control1.y);
...
export function quadraticCurveToPoints(
	curve: QuadraticCurveProperties,
	segments: number = 20
): PointProperties[] {
