...
export function sortShapesInDirection(shapes: Shape[], direction: DirectionEnum): Shape[] {
	...
	for (let i = 0; i < n; i++) {
		...
		if (shapeMap.has(keyAB)) {
			...
			throw new Error(`No shape connects point ${pointA.key} to point ${pointB.key}`);
		}
	}
...
export function chainBoundary(chain: Chain): Boundary {
 ...
 * @returns
 */
function pointEnclosedByPolygon(
	point: PointProperties,
	polygon: PointProperties[],
