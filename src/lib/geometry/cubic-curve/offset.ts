...
export class Point {
	...
	equalsWithEpsilon(point: Point, epsilon: number): boolean {
		return (
			isEqualWithEpsilon(point.X, this.X, epsilon) && isEqualWithEpsilon(point.Y, this.Y, epsilon)
		);
	}
...
class Segment {
	 ...
	 */
	isPoint(): boolean {
		return IsEqualish(this.P0.X, this.P1.X) && IsEqualish(this.P0.Y, this.P1.Y);
	}
	...
	extendByLengthFront(length: number) {
		...
		this.P1 = new Point(this.P1.X + v.X * length, this.P1.Y + v.Y * length);
	}
	...
	extendByLengthBack(length: number) {
		...
		this.P0 = new Point(this.P0.X - v.X * length, this.P0.Y - v.Y * length);
	}
...
class SegmentIntersection {
	static noIntersection: SegmentIntersection = new SegmentIntersection(
...
export class CubicCurve {
	...
	static createFromQuadratic(p0: Point, p1: Point, p2: Point): CubicCurve {
		return new CubicCurve(
			p0,
			new Point(p0.X + (2.0 / 3.0) * (p1.X - p0.X), p0.Y + (2.0 / 3.0) * (p1.Y - p0.Y)),
			new Point(p1.X + (1.0 / 3.0) * (p2.X - p1.X), p1.Y + (1.0 / 3.0) * (p2.Y - p1.Y)),
			p2
		);
	}
	...
	static createFromLine(p0: Point, p1: Point): CubicCurve {
		return new CubicCurve(
			p0,
			new Point(p0.X + (1.0 / 3.0) * (p1.X - p0.X), p0.Y + (1.0 / 3.0) * (p1.Y - p0.Y)),
			new Point(p0.X + (2.0 / 3.0) * (p1.X - p0.X), p0.Y + (2.0 / 3.0) * (p1.Y - p0.Y)),
			p1
		);
	}
	...
	split(): Array<CubicCurve> {
		...
		return [new CubicCurve(this.P0, aP2, aP3, m), new CubicCurve(m, bP2, bP3, this.P3)];
	}
...
class OutputBuilder {
	...
	arcTo(center: Point, to: Point, clockwise: boolean) {
		...
		const determinedOrientation = Point.determineTriangleOrientation(center, arcFrom, to);
		...
		for (let i = 1; i <= nSteps; i++) {
			...
			const unitCurve = this.findUnitCubicCurveForArc(new Point(c, s), new Point(c1, s1));
	...
	arcOffset(offset: number, center: Point, from: Point, to: Point, clockwise: boolean) {
		...
		// arc orientation.
		this.arcTo(center, l2.P1, Point.isTriangleClockwise(center, this.previousPoint, l2.P1));
	}
}

export function offsetCubicCurve(
	curve: CubicCurve,
	offset: number,
	maximumError: number
): CubicCurve[] {
...
function acceptOffset(
	...
	if (
		Point.isTriangleClockwise(original.P0, original.P1, original.P3) !=
	...
	if (
		Point.isTriangleClockwise(original.P0, original.P2, original.P3) !=
...
class CurveTangentData {
	...
	constructor(curve: CubicCurve) {
		...
		this.turn1 = unitTurn(this.startTangent.P0, this.startTangent.P1, this.endTangent.P0);
		this.turn2 = unitTurn(this.startTangent.P0, this.endTangent.P1, this.endTangent.P0);
...
function tryArcApproximation(
	...
	const G = curve.P0.multiplyScalar(P3VDistance)
		.plus(curve.P3.multiplyScalar(P0VDistance).plus(V.multiplyScalar(P0P3Distance)))
	...
	if (C1.equalsWithEpsilon(C2, ArcCenterComparisonEpsilon)) {
		...
		if (goodArc(C1, radius, curve, maximumError, 0, 1)) {
			const clockwise = Point.isTriangleClockwise(curve.P0, V, curve.P3);
...
function offsetLinearCuspyCurve(
	...
	builder: OutputBuilder,
	offset: number,
	maximumCurvaturePoints: Array<number>
) {
	...
	for (let i = 0; i < maximumCurvaturePoints.length; i++) {
		...
		if (lengthSquared <= 1e-9) {
			...
			builder.arcTo(
				pointAtCusp,
				aTo,
				Point.isTriangleClockwise(previousPoint, previousOffsetPoint, pointAtCusp)
			);
