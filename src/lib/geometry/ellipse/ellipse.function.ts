...
import { type CubicCurveProperties } from '../cubic-curve/cubic-curve';
import { type EllipseProperties } from './ellipse';
...
export function ellipseToCubicCurves({
	...
	startAngle,
	endAngle
}: EllipseProperties): CubicCurveProperties[] {
	const curves = [];
...
export function ellipseMiddlePoint(ellipse: EllipseProperties): PointProperties {
	const { center, focus, axisRatio, startAngle, endAngle } = ellipse;
	...
	const x = center.x + a * cosMidAngle * Math.cos(rotation) - b * sinMidAngle * Math.sin(rotation);
	const y = center.y + a * cosMidAngle * Math.sin(rotation) + b * sinMidAngle * Math.cos(rotation);
